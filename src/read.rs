...
static TR_KEY: Lazy<tag::Tag> = Lazy::new(|| "TR".parse().unwrap());
...
impl ReadInfo {
    ...
    pub fn new(record: Record, locus: &Locus) -> Result<Option<Self>> {
        ...
        let trid = match data.get(&*TR_KEY) {
            ...
            _ => {
                return Err(anyhow!(
                    ...
                    record.read_name().unwrap().to_string()
                ));
            }
        };
        ...
        let name = record.read_name().unwrap().to_owned().to_string();
        ...
        let pos = record.alignment_start().unwrap().get();
...
impl ReadInfoBuilder {
    ...
    /// Sets the name for the `ReadInfo` being built.
    ///
    /// # Arguments
    ///
    ...
    pub fn build(self) -> Result<ReadInfo> {
        Ok(ReadInfo {
            name: self.name,
            bases: self.bases,
